...
function parseRelease(data) {
    ...
    $.each(data.tracks.items, function(index, trackobj) {
        ...
        let performers = trackobj.performers.split('\r - ').map(function(v) {
            let list = v.split(', ');
            let name = list.shift();
            return [name, list];
        });
...
function insertLink(release) {
    ...
    let mbUI = $(`<p class="musicbrainz-import">${MBImport.buildFormHTML(parameters)}${MBImport.buildSearchButton(release)}</p>`).hide();
